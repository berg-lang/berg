...
pub fn term<'c, 'b>(parser: &mut Parser<'c, 'b>) -> bool {
    ...
    if index >= parser.buffer.len() { return false; }
...
pub fn invalid_or_unsupported<'c, 'b>(parser: &mut Parser<'c, 'b>, mut index: ByteIndex) -> bool {
    ...
    while index < parser.buffer.len() {
        match parser.buffer[index] {
            UTF8_CONT_START..UTF8_2_START | UTF8_INVALID_START..0xFF => { index += 1; },
            _ => break,
        }
    }
...
fn invalid_utf8_error<'c, 'b>(error_type: CompileErrorType, parser: &mut Parser<'c, 'b>, end: ByteIndex) -> bool {
...
fn error<'c, 'b>(error_type: CompileErrorType, parser: &mut Parser<'c, 'b>, end: ByteIndex) -> bool {
    let start = parser.index;
    parser.index = end;
...
fn token<'c, 'b>(expression_type: SyntaxExpressionType, parser: &mut Parser<'c, 'b>, end: ByteIndex) -> bool {
    let start = parser.index;
    parser.index = end;
    let buf = parser.buffer[start..end].to_vec();
    let string = unsafe { String::from_utf8_unchecked(buf) };
    parser.expressions.push(SyntaxExpression::new(expression_type, start, string));
    true
}
