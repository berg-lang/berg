...
compile_errors! {
    ...
    pub struct LeftSideOfAssignmentMustBeIdentifier { pub left: ByteRange, pub operator: ByteRange } (305) = format(left, "The assignment operator '{operator}' must have a field declaration or name on the left side (like \":foo {operator} ...\" or \"foo {operator} ...\"): the left side is currently {left}.");
    pub struct LeftSideOfIncrementOrDecrementMustBeIdentifier { pub left: ByteRange, pub operator: ByteRange } (305) = format(left, "The assignment operator '{operator}' must have a field name on the left side (like \"foo{operator}\"): the left side is currently '{left}'.");
    pub struct RightSideOfIncrementOrDecrementMustBeIdentifier { pub right: ByteRange, pub operator: ByteRange } (305) = format(right, "The assignment operator '{operator}' must have a field name on the right side (like \"{operator}foo ...\" or \"{operator}foo ...\"): the right side is currently {right}.");
    ...
    pub struct NoSuchField          { pub reference: ByteRange } (1005) = format(reference, "No such field: '{reference}'");
    pub struct FieldNotSet          { pub reference: ByteRange } (1006) = format(reference, "Field '{reference}' was declared, but never set to a value!");
