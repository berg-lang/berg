...
impl Debug for SourceSpec {
    fn fmt(&self, f: &mut Formatter) -> fmt::Result {
        match *self {
            SourceSpec::File { ref path } => f.debug_struct("SourceSpec::File").field("path", path).finish(),
            SourceSpec::Memory { ref name, ref contents } => f.debug_struct("SourceSpec::Memory").field("name",name).field("contents",contents).finish(),
        }
    }
}
...
impl SourceSpec {
    ...
    pub(crate) fn open<'a>(&'a self, compiler: &Compiler, parse_result: &mut ParseResult) -> Cow<'a, ByteSlice> {
    ...
    pub(crate) fn open_and_report<'a>(&'a self, compiler: &Compiler, mut parse_result: Option<&mut ParseResult>) -> Cow<'a, ByteSlice> {
    ...
    fn open_file(compiler: &Compiler, path: &PathBuf, parse_result: &mut Option<&mut ParseResult>) -> Cow<'static, [u8]> {
        match compiler.absolute_path(path) {
                    ...
                    Ok(mut file) => {
                        ...
                        if let Err(error) = file.read_to_end(&mut buffer) {
                            for parse_result in parse_result.iter_mut() {
                                let source = parse_result.index;
                                parse_result.report_open_error(IoReadError::value(source, path.clone(), error.to_string()));
                            }
                        }
                        Cow::Owned(buffer)
                    },
            ...
            Err(io_error_string) => {
                if let Some(ref mut parse_result) = *parse_result {
                    let source = parse_result.index;
                    let error = IoCurrentDirectoryError::value(source, path.clone(), io_error_string);
