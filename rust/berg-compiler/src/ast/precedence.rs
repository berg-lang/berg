...
impl Precedence {
    pub(crate) fn takes_right_child(self, right: Precedence) -> bool {
        match self {
            ...
            Or                => match right { TimesDivide|PlusMinus|Comparison|And => true, _ => false },
            Assign            => match right { TimesDivide|PlusMinus|Comparison|And|Or => true, _ => false },
            SemicolonSequence => match right { TimesDivide|PlusMinus|Comparison|And|Or|Assign => true, _ => false },
            NewlineSequence   => match right { TimesDivide|PlusMinus|Comparison|And|Or|Assign|SemicolonSequence => true, _ => false },
        }
    }
