...
impl Scanner {
    ...
    pub fn next_while(&mut self, matcher: impl CharMatcher) -> Option<CharType> {
        if let Some(char_type) = self.next_if(matcher) {
            let mut result = char_type;
            while let Some(char_type) = self.next_if(matcher) {
                result = char_type;
            }
            return Some(result);
        }
    ...
    pub fn next_until(&mut self, matcher: impl CharMatcher) -> Option<CharType> {
        if let Some(char_type) = self.next_unless(matcher) {
            let mut result = char_type;
            while let Some(char_type) = self.next_unless(matcher) {
                result = char_type;
            }
            return Some(result);
        }
    ...
    pub fn next_if(&mut self, matcher: impl CharMatcher) -> Option<CharType> {
        let (char_type, char_length) = CharType::read(self.buffer(), self.index);
    ...
    pub fn next_unless(&mut self, matcher: impl CharMatcher) -> Option<CharType> {
        let (char_type, char_length) = CharType::read(self.buffer(), self.index);
        if matcher.matches(char_type) {
            None
        } else {
