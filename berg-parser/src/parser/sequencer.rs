...
impl Sequencer {
    ...
    pub fn parse(mut self) -> Ast {
        ...
        loop {
            ...
            match char_type {
                ...
                OtherOperator | Dash => self.operator(start),
                Equal => self.equal(start),
                ExclamationPoint | LessThan | GreaterThan => self.comparison_operator_start(start),
    ...
    fn integer(&mut self, start: ByteIndex) {
        ...
        if self
            .scanner
            .next_while(&CharType::is_identifier_middle)
            .is_some()
        {
    ...
    fn operator(&mut self, start: ByteIndex) {
        if let Some(Equal) = self.scanner.next_while(&CharType::is_operator) {
    ...
    fn equal(&mut self, start: ByteIndex) {
        ...
        if self.scanner.next_if(&CharType::is_operator).is_none() {
            self.emit_assignment_operator(start);
        } else {
            self.operator(start);
        }
    ...
    fn comparison_operator_start(&mut self, start: ByteIndex) {
        // Check for !=, >=, <= -- none of these are assignment operators
        match self.scanner.next_if(&CharType::is_operator) {
            // !, <, >
            None => self.emit_operator(start),
            // !=, <=, >=
            Some(Equal) if self.scanner.next_if(&CharType::is_operator).is_none() => {
                self.emit_operator(start)
            }
            // Anything else starting with !, < or >
            Some(_) => self.operator(start),
        }
    }
    ...
    fn read_space(&mut self, start: ByteIndex) -> Option<WhitespaceIndex> {
        if self
            .scanner
            .next_while(&CharType::is_horizontal_whitespace)
            .is_some()
        {
